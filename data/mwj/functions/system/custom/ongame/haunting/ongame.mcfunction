## Haunting
execute unless entity @e[type=minecraft:armor_stand,tag=HauntPlayer_Teleporter] as @a[scores={HauntPlayer=1}] run title @s title "成仏できません"
execute as @a[scores={HauntPlayer=1}] run gamemode adventure @s
execute unless entity @e[type=minecraft:armor_stand,tag=HauntPlayer_Teleporter] as @a[scores={HauntPlayer=1}] at @s run summon minecraft:armor_stand ~ ~ ~ {Marker:1,Invisible:1,Tags:["HauntPlayer_Teleporter"],Silent:1b}
