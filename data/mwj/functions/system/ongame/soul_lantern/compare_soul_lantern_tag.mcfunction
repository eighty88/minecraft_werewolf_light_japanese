...
execute store success score @s TeleporterTemp run data modify entity @e[type=item,nbt={Item:{id:"minecraft:soul_lantern",tag:{Tags:["MWLitem"]}}},sort=nearest,limit=1] Item.tag.display.Lore[3] set from storage mwj:teleporter Teleporter.Temp.Message

## If not equal soul_lantern's tag to temporary storage, return soul_lantern's tag
execute if score @s TeleporterTemp matches 1 run data modify entity @e[type=item,nbt={Item:{id:"minecraft:soul_lantern",tag:{Tags:["MWLitem"]}}},sort=nearest,limit=1] Item.tag.display.Lore[3] set from storage mwj:teleporter Teleporter.SoulLantern
